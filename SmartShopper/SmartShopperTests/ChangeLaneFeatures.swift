...
class ChangeLanes: XCTestCase {
    ...
    func testItemAisleChanged() {
        
        let result: ReturnCode = Store.shared.changeAisle(for: "024100122332", in: ChangeLanes.departmentList[0], toThisAisle: 4)
        
        XCTAssertTrue(result == ReturnCode.itemAisleNumChanged, "error: \(result)")
    }
    
    func testItemNotInStore(){
        let itemNotFoundInStore: ReturnCode = Store.shared.changeAisle(for: "024100165678", in: ChangeLanes.departmentList[0], toThisAisle: 4)
        
        //let result: ReturnCode = Store.shared.changeAisle(for: "024100122332", in: ChangeLanes.departmentList[0], toThisAisle: 5)
        XCTAssertTrue(itemNotFoundInStore == ReturnCode.itemNotInStore)
    }
    
    func testItemBarcodeInvalid(){
        
        let itemBarcodeInvalid: ReturnCode = Store.shared.changeAisle(for: "02410678", in: ChangeLanes.departmentList[0], toThisAisle: 4)
        
        XCTAssert(itemBarcodeInvalid == ReturnCode.invalidBarcodelength, "error: \(itemBarcodeInvalid)")
    }
    
    func testAisleOutOfRange(){
        let aisleOutOfRange: ReturnCode = Store.shared.changeAisle(for: "024100122332", in: ChangeLanes.departmentList[0], toThisAisle: 50)
        XCTAssert(aisleOutOfRange == ReturnCode.aisleOutOfRangeForDeparment, "error: \(aisleOutOfRange)")
    }
